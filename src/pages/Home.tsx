...
function SectionHeader({ label, title, titleHighlight, subtext }: { label: string, title: string, titleHighlight?: string, subtext: string }) {
  return (
    ...
    >
      <div className="text-primary font-bold tracking-widest uppercase text-xs mb-2">{label}</div>
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-background font-sans overflow-x-hidden">
      ...
      {/* STATS SECTION */}
      <section className="py-6 bg-white border-y border-border relative z-20">
      ...
      <section id="about" className="py-12 md:py-16 bg-[#fdf9f3]">
        <div className="container mx-auto px-4 md:px-8 max-w-6xl">
          <div className="grid md:grid-cols-2 gap-10 items-center">
            ...
            >
              {/* Main card with logo */}
              <div className="w-64 h-64 md:w-80 md:h-80 rounded-3xl bg-white flex items-center justify-center shadow-2xl relative overflow-hidden border border-border">
                ...
                <div className="absolute inset-6 border border-primary/10 rounded-2xl" />
                {/* Actual logo */}
                <img
                  src={logoPath}
                  alt="Nuqta Creative Studio"
                  className="w-44 md:w-56 object-contain relative z-10"
                />
              </div>
      ...
      {/* HOW IT WORKS */}
      <section id="process" className="py-10 md:py-24 bg-background">
        <div className="container mx-auto px-4 md:px-8">
          <SectionHeader 
            label="Our Process"
            title="How It Works"
          ...
          <div className="relative max-w-5xl mx-auto">
            <div className="hidden md:block absolute top-10 left-[10%] right-[10%] h-0.5 border-t-2 border-dashed border-border z-0" />
            ...
            >
              {[
                ...
                <motion.div key={i} variants={fadeInUp} className="text-center">
                  <div className="w-16 h-16 bg-primary text-white rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg relative">
                    <item.icon size={28} />
                    <div className="absolute -top-2 -right-2 bg-foreground text-background text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center border-2 border-background">
                      {item.step}
                    </div>
                  </div>
      ...
      {/* OUR SERVICES */}
      <section id="services" className="py-16 md:py-14 bg-background">
        <div className="container mx-auto px-4 md:px-8">
          <SectionHeader 
            label="What We Do"
            title="Our Services"
      ...
      {/* TOOLS & PLATFORMS */}
      <section className="py-10 md:py-12 bg-white border-y border-border overflow-hidden">
        <div className="container mx-auto px-4 md:px-8 text-center">
          <motion.div variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="mb-8">
            <div className="text-primary font-bold tracking-widest uppercase text-xs mb-2">Tools & Platforms</div>
            <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-2">Powered by <span className="text-primary">Industry-Leading</span> Platforms</h2>
            <p className="text-muted-foreground text-sm">We work with the world's top tools and platforms to deliver outstanding results.</p>
          </motion.div>
          ...
          >
            {[
              ...
              <motion.div key={i} variants={{ hidden: { opacity: 0, x: -20 }, visible: { opacity: 1, x: 0 } }} className="flex flex-col items-center gap-2 group">
                <tool.icon size={36} className="text-foreground group-hover:text-primary transition-colors" />
                <span className="text-xs font-medium text-muted-foreground group-hover:text-foreground">{tool.name}</span>
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      <section id="contributions" className="py-14 md:py-20 bg-background overflow-hidden">
        <div className="container mx-auto px-4 md:px-8">
          <SectionHeader
            label=""
        ...
        {/* Marquee row 1 — first half of logos, scrolls left */}
        ...
        {/* Marquee row 2 — second half of logos, scrolls right */}
        <div className="relative">
          <div className="absolute left-0 top-0 bottom-0 w-24 bg-gradient-to-r from-background to-transparent z-10 pointer-events-none" />
          <div className="absolute right-0 top-0 bottom-0 w-24 bg-gradient-to-l from-background to-transparent z-10 pointer-events-none" />

          <div className="flex gap-16 animate-marquee-right whitespace-nowrap items-center">
            {[...clientLogos.slice(9), ...clientLogos.slice(9)].map((client, i) => (
              <div key={i} className="inline-flex items-center justify-center shrink-0">
                <img
                  src={client.img}
                  alt={client.name}
        ...
        <motion.p
          ...
          className="text-center text-muted-foreground mt-10 text-sm container mx-auto px-4"
        >
      ...
      {/* RECENT PROJECTS */}
      <section id="projects" className="py-14 md:py-12 bg-white">
        <div className="container mx-auto px-4 md:px-8">
          <motion.div variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="text-center max-w-2xl mx-auto mb-10">
            <div className="text-primary font-bold tracking-widest uppercase text-xs mb-3">Our Work</div>
            <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-3">Recent <span className="text-primary">Projects</span></h2>
            <p className="text-muted-foreground text-sm">A glimpse into the brands we've helped build, grow, and stand out.</p>
          </motion.div>
      ...
      {/* PRICING */}
      <section className="py-14 md:py-12 bg-white">
        <div className="container mx-auto px-4 md:px-8">
          <motion.div variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="text-center max-w-2xl mx-auto mb-10">
          ...
          >
            {[
              ...
              <motion.div key={i} variants={fadeInUp} className="bg-white p-5 rounded-2xl shadow-sm border border-border flex flex-col">
                ...
                <p className="text-sm italic text-foreground mb-4 grow">"{testimonial.text}"</p>
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-primary text-white flex items-center justify-center font-bold text-sm shrink-0">{testimonial.initials}</div>
                  <div>
          ...
          <Accordion type="single" collapsible className="w-full">
            {[
              ...
              <AccordionItem key={i} value={`item-${i}`} className="border-b border-border py-1">
                <AccordionTrigger className="text-sm font-bold hover:text-primary hover:no-underline transition-colors text-left py-3">{faq.q}</AccordionTrigger>
                <AccordionContent className="text-muted-foreground text-sm leading-relaxed pb-3">{faq.a}</AccordionContent>
              </AccordionItem>
            ))}
          </Accordion>
        </div>
      ...
      <section id="contact" className="py-10 md:py-14 bg-background border-t border-border">
        <div className="container mx-auto px-4 md:px-8 max-w-5xl text-center">
          ...
          <div className="grid md:grid-cols-3 gap-5">
            <motion.a href="https://wa.me/923297646980" target="_blank" rel="noopener noreferrer" whileHover={{ y: -4 }} className="bg-white p-5 rounded-2xl border border-border shadow-sm flex flex-col items-center group cursor-pointer">
              <div className="w-14 h-14 rounded-full bg-primary flex items-center justify-center text-white mb-3 group-hover:scale-110 transition-transform"><Phone size={24} /></div>
              <h3 className="text-base font-bold mb-1">WhatsApp</h3>
              <p className="text-muted-foreground text-sm">0329-7646980</p>
            </motion.a>

            <motion.a href="mailto:nuqtacreativestudiopk@gmail.com" whileHover={{ y: -4 }} className="bg-white p-5 rounded-2xl border border-border shadow-sm flex flex-col items-center group cursor-pointer">
              <div className="w-14 h-14 rounded-full bg-primary flex items-center justify-center text-white mb-3 group-hover:scale-110 transition-transform"><Mail size={24} /></div>
              <h3 className="text-base font-bold mb-1">Email</h3>
              <p className="text-muted-foreground text-sm">nuqtacreativestudiopk@gmail.com</p>
            </motion.a>

            <motion.a href="https://instagram.com/nuqtacreativestudio1" target="_blank" rel="noopener noreferrer" whileHover={{ y: -4 }} className="bg-white p-5 rounded-2xl border border-border shadow-sm flex flex-col items-center group cursor-pointer">
              <div className="w-14 h-14 rounded-full bg-primary flex items-center justify-center text-white mb-3 group-hover:scale-110 transition-transform"><Instagram size={24} /></div>
              <h3 className="text-base font-bold mb-1">Instagram</h3>
              <p className="text-muted-foreground text-sm">@nuqtacreativestudio1</p>
            </motion.a>
          </div>
        </div>
      </section>
      ...
      {/* FOOTER */}
      <footer className="bg-white border-t border-border pt-10 pb-6">
        <div className="container mx-auto px-4 md:px-8 max-w-7xl">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
            ...
            <div>
              <h4 className="text-sm font-bold mb-3 text-foreground">Services</h4>
              <ul className="space-y-2 text-muted-foreground text-xs">
            ...
            <div>
              <h4 className="text-sm font-bold mb-3 text-foreground">Contact</h4>
              <ul className="space-y-2 text-muted-foreground text-xs">
                <li className="flex items-center gap-2"><Phone size={14} className="text-primary shrink-0" /><span>0329-7646980</span></li>
                <li className="flex items-center gap-2"><Mail size={14} className="text-primary shrink-0" /><span>nuqtacreativestudiopk@gmail.com</span></li>
                <li className="flex items-center gap-2"><Instagram size={14} className="text-primary shrink-0" /><span>@nuqtacreativestudio1</span></li>
              </ul>
            </div>
          </div>
          
          <div className="border-t border-border pt-2 text-center text-muted-foreground text-xs flex flex-col md:flex-row justify-between items-center gap-2">
